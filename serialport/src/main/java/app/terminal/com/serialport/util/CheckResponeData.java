...
public class CheckResponeData {
    ...
    public static String getErrorInfo(byte[] data, int currentOrder) {
        if (data.length >= 7 && data[6] == 0) {
            return "";
        } else if (data.length >= 7 && (data[0] == 0x55) && (data[1] == 0x55)) {
            if ((data[6] != 0) && (currentOrder == 26 || currentOrder == 29)) {
                return "#↑密钥A验证失败！";
            } else if ((data[6] != 0) && (currentOrder == 27 || currentOrder == 30)) {
                return "#↑密钥B验证失败！";
            }
            switch (data[6]) {
                case 0x3B:
